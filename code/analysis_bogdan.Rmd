...
- duplicates entries for different subfields have been added somehow!
...
Additional articles are in the "dodgy papers + large legit effect sizes" zotero folder, not all details of articles i looked into are below.
...
d = 2.5 in a component study included in this meta-analysis of Psychedelic therapy for depressive symptoms. 
...
d_percentiles <- data_percentiles_for_plot |>
...
ggsave(d_percentiles, "plots/percentiles_bogdan_d.png", width = 6, height = 4, dpi = 600)
...
dat_t <- read_rds("../data/bogdan 2025/processed/dat_t.rds")

# note that `r .Machine$double.xmin` is the floating point limit, but floating point weirdness can occur before this, so all order of magnitude >300 (including p == 0, which is a value smaller than the fp limit) are coded as 300. 
# 308 is the technical limit, 300 is a safe clean number.
cap <- 300 

dat_t_processed <- dat_t |>
  mutate(
    order_of_magnitude = case_when(
      p_implied == 0 ~ cap,                      # Handle absolute 0
      p_implied < 10^-cap ~ cap,                 # Cap extremely small subnormals
      TRUE ~ abs(floor(log10(p_implied)))        # Standard calculation
    )
  ) |>
  mutate(n = df + 2) |>
  mutate(cohens_d = (2 * t) / sqrt(df))


```

## Percentiles of order of magnitude

```{r}

percentiles_vec <- c(.5, .6, .7, .8, .9, .95, .99, .999)

dat_t_processed %>%
  reframe(
    percentile = percentiles_vec,
    value = quantile(order_of_magnitude, percentiles_vec, na.rm = TRUE)
  )
...
percentiles_vec <- c(.001, .01, .1, .2, .3, .4, .5, .6, .7, .8, .9, .95, .99, .999)

dat_t_processed %>%
  reframe(
    percentile = percentiles_vec,
    value = quantile(n, percentiles_vec, na.rm = TRUE)
  )

ggplot(dat_t_processed, aes(n)) +
  geom_histogram() +
...
ggplot(dat_t_processed, aes(n, order_of_magnitude)) +
  geom_point(alpha = 0.2) +
  geom_hline(yintercept = 17, color = "purple", linetype = "dashed") +
  scale_x_log10()

dat_t_processed |>
  filter(n >= 10 & n <= 1000) |>
  ggplot(aes(n, order_of_magnitude)) +
  geom_point(alpha = 0.2) +
  geom_hline(yintercept = 17, color = "purple", linetype = "dashed") +
  scale_x_log10()

library(hexbin) # Required for geom_hex()

dat_t_processed |>
  filter(n >= 10 & n <= 1000) |>
  ggplot(aes(n, order_of_magnitude)) +
  # Replaced geom_point with geom_hex
  geom_hex(bins = 30) + 
  geom_hline(yintercept = 17, color = "lightgreen", linetype = "dashed") +
  scale_x_log10(labels = scales::label_comma()) +
...
dat_t_processed |>
  ...
  geom_hline(yintercept = 17, color = "lightgreen", linetype = "dashed") +
  scale_x_log10(labels = scales::label_comma()) +
  # Apply log10 transformation to the fill (density count)
...
dat_t_processed |>
  filter(n >= 10 & n <= 1000) |>
  ggplot(aes(cohens_d, order_of_magnitude)) +
  geom_hex(bins = 30) +
  geom_hline(yintercept = 17, color = "lightgreen", linetype = "dashed") +
  scale_x_log10(labels = scales::label_comma()) +
  # Apply log10 transformation to the fill (density count)
...
dat_d_native <- data_effectsizes_possible |>
  filter(type=="d_native")
