...
# TODO add the stanley et all 2018 data in here!
...
data_percentiles_bogdan <- read_rds("../data/bogdan 2025/processed/data_effectsizes.rds") |>
  ...
  select(percentile,
         ...
         d_z,
         r = abs_r) |>
  mutate(d_s = round_up(d_s, digits = 2))

data_percentiles_cuijpers <- read_csv("../data/cuijpers metapsy psy vs control/metapsy_depression_psy_vs_control_effect_sizes_2024-01-01.csv") |>
  summarise(
    percentile = c(1, 5, 10, 20, 25, 30, 40, 50, 60, 70, 75, 80, 90, 95, 99) / 100,
    d = map_dbl(percentile, ~ quantile(hedges_g_absolute, probs = .x, na.rm = TRUE)),
    .groups = "drop"
  ) |>
  mutate(percentile = percentile * 100)

dat_percentiles_comparative <- bind_rows(
  data_percentiles_cuijpers |>
    mutate(Source = "Cuijpers et al. (2024)"),
  ...
  data_percentiles_bogdan |>
    select(percentile,
           d = d_native) |> 
    mutate(Source = "Bogdan (2025) reported"),
  
  data_percentiles_bogdan |>
    select(percentile,
           d = d_s) |> 
    mutate(Source = "Bogdan (2025) recalculated"),
  ...
  read_csv("../data/previous articles extracting many effect sizes/stanley et al. 2018/outputs/percentiles_by_metric.csv") |> 
    mutate(Source = "Stanley et al. (2018)") |>
    select(-yi_raw) |>
    pivot_wider(names_from = metric, values_from = yi_abs)
) |>
...
data_table_r <- dat_percentiles_comparative |>
  select(percentile, Source, r) |>
  mutate(r = round_up(r, 2)) |>
  drop_na() |>
...
dat_percentiles_comparative |>
  ...
  geom_smooth(method = "gam", size = 0.5, se = FALSE) +
  #geom_line() +
  geom_point() +
  scale_y_continuous(breaks = c(1, seq(5, 95, 5), 99), name = "Percentile") +
  scale_x_continuous(breaks = scales::breaks_pretty(n = 10), limits = c(0, 1), 
                     name = expression("Pearson's" ~ group("|", italic(r), "|"))) +
  theme_linedraw() +
...
data_table_d <- dat_percentiles_comparative |>
  select(percentile, Source, d) |>
  mutate(d = round_up(d, 2)) |>
  drop_na() |>
  pivot_wider(names_from = "Source",
              values_from = "d") |>
  arrange(desc(percentile)) 
...
dat_percentiles_comparative |>
  ...
  scale_x_continuous(breaks = c(0, .2, .5, .8, 1, 1.5, 2, 2.5, 3, 3.5), 
