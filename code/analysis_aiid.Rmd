---
title: "Differences and correlations between categories and personality scale items - AIID data"
author: "Ian Hussey"
...
## Correlations and cohen's d between concept pairs

study presented a pair of concepts to each participant, who had to separately rate their "actual feelings towards each" on separate 1-10 likert scales. 
...
dat_r_d_all <- data_trimmed |>
  ...
  summarise(n = n(), 
            ...
            r_actuals = broom::tidy(cor.test(actual_y, actual_x)),
            d = cohens_d(actual_y, actual_x, pooled = TRUE)) |>
  ...
  select(domain, n, 
         #mean_actual_x, mean_actual_y, 
         #sd_actual_x, sd_actual_y, 
         r = r_actuals_estimate, 
         r_lower = r_actuals_conf.low, 
         r_upper = r_actuals_conf.high,
         d = Cohens_d, 
         d_lower = CI_low, 
         d_upper = CI_high) |>
  mutate(d_abs = abs(d),
...
ggplot(dat_r_d_all, aes(d_abs)) +
  geom_histogram(binwidth = 0.1, boundary = 0) +
  xlim(0, NA)

ggplot(dat_r_d_all, aes(d)) +
  geom_histogram(binwidth = 0.1, boundary = 0) 

ggplot(dat_r_d_all, aes(r)) +
  geom_histogram(binwidth = 0.1, boundary = 0) +
  xlim(-1, 1)

ggplot(dat_r_d_all, aes(r, d_abs)) +
...
histogram of the domain with the largest SD that is above the horns cutoff

```{r}

data_trimmed |>
  filter(domain == "Evolution - Creationism") |>
  drop_na(actual_y) |>
  summarize(sd_y = sd(actual_y))
