######################################################################
### Analysis as described in https://www.researchgate.net/publication/379447785
### Illustrated with a DOSPERT dataset
### Load a new dataset to run a new analysis

################################################################
### Frey et al. (2023) https://link.springer.com/article/10.1007/s11166-022-09398-5
### data https://osf.io/pjt57/

# clean memory and read the dataset (in this case a csv file)
rm(list = ls())
DOSPERT_Frey <- read.csv("DOSPERT_Frey.csv")

# make a new dataset (mydata) limited to the colomns of the test
# and check whether everything is OK (also number of response alternatives)
...
# Do the analysis with polychoric correlation
...
# Give solution with 1 factor polychoric
fit4 <- fa(rho,1,n.obs=nrow(mydata))
fit4
diagram(fit4)
# %variance explained=.25, RMSEA=.147, RMSR=.13, TLI=.385

# run exploratory graph analysis to get an idea of the number of clusters
...
### run confirmatory factor analyses
...
'
# run analysis with ordered=TRUE
...
# repeat the analysis with estimatory="MLR"
...
# run CFA analyses with 5 factors (items assigned to factors according to theory)
colnames(mydata)
EGAmodel= '
...
# model with orthogonal factors (only MLR)
...
# extra Bifactor model (not covered in manuscript)
BIFmodel= '
 ethical   =~ ethR_1+ethR_2+ethR_3+ethR_4+ethR_5+ethR_6
 financial =~ finR_1+finR_2+finR_3+finR_4+finR_5+finR_6
 health    =~ heaR_1+heaR_2+heaR_3+heaR_4+heaR_5+heaR_6
...
# calculate ECV measures (for more info, see https://drjosephhammer.com/research/bifactor-analysis-resources/)
if (!require(BifactorIndicesCalculator)) install.packages('BifactorIndicesCalculator')
